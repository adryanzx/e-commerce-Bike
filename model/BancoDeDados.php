    ...
    if (mysqli_query($conexao, $consulProd)) {
       ...
       echo $produto->get_Tipo(); 
        ...
        switch (get_class($produto)) {
            case 'Bike':
                $quadro = $produto->getQuadro();
                $consulEspecifico = "INSERT INTO bike (produto_cod, quadro) VALUES ('$produtoId', '$quadro')";
                if (!mysqli_query($conexao, $consulEspecifico)) {
                    echo "Erro ao inserir na tabela bike: " . mysqli_error($conexao);
                }
                break;
            case 'sapatilha':
                $marca = $produto->getMarca();
                $consulEspecifico = "INSERT INTO sapatilha (produto_cod, marca) VALUES ('$produtoId', '$marca')";
                if (!mysqli_query($conexao, $consulEspecifico)) {
                    echo "Erro ao inserir na tabela sapatilha: " . mysqli_error($conexao);
                }
                break;
            case 'oculos':
                $cor = $produto->getCor();
                $consulEspecifico = "INSERT INTO oculos (produto_cod, cor) VALUES ('$produtoId', '$cor')";
                if (!mysqli_query($conexao, $consulEspecifico)) {
                    echo "Erro ao inserir na tabela oculos: " . mysqli_error($conexao);
                }
                break;
